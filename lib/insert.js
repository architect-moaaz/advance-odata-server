...
function processBody (data, cfg, req, res) {
  try {
    ...
    cfg.executeInsert(req.params.collection, data, req, function (err, entity) {
      ...
      let entity_id = entity.insertedIds[0];
      res.statusCode = 201
      ...
      res.setHeader('Location', cfg.serviceUrl + '/' + req.params.collection + "/('" + encodeURI(entity_id) + "')")
      cfg.addCorsToResponse(res)

      cfg.pruneResults(req.params.collection, entity)

      // odata.context must be first
      entity['@odata.id'] = cfg.serviceUrl + '/' + req.params.collection + "('" + entity_id+ "')"
      entity['@odata.editLink'] = cfg.serviceUrl + '/' + req.params.collection + "('" + entity_id + "')"
